    ...
    <div class="py-12">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                ...
                <form method="POST" action="{{ route('admin.exam-configurations.update', $examConfiguration) }}">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-4">
                        ...
                        <select name="program_id" class="block w-full border-gray-300 rounded-md text-sm">
                            @foreach ($programs as $program)
                                <option value="{{ $program->id }}" {{ old('program_id', $examConfiguration->program_id) == $program->id ? 'selected' : '' }}>
                                    {{ $program->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="name" value="{{ old('name', $examConfiguration->name) }}"
                               class="block w-full border-gray-300 rounded-md text-sm">
                    </div>

                    <div class="mb-4">
                        <label class="block text-sm font-medium text-gray-700 mb-1">Duration (minutes)</label>
                        <input type="number" name="duration_minutes" value="{{ old('duration_minutes', $examConfiguration->duration_minutes) }}"
                               class="block w-full border-gray-300 rounded-md text-sm">
                    </div>
                    ...
                    <div class="mb-6 border-t pt-4">
                        ...
                        <div class="grid grid-cols-3 gap-3">
                            <div>
                                ...
                                <input type="number" step="0.01" name="marks_correct" value="{{ old('marks_correct', $examConfiguration->marks_correct) }}"
                                       class="block w-full border-gray-300 rounded-md text-sm">
                            </div>
                            <div>
                                <label class="block text-xs text-gray-600 mb-1">Wrong Answer</label>
                                <input type="number" step="0.01" name="marks_wrong" value="{{ old('marks_wrong', $examConfiguration->marks_wrong) }}"
                                       class="block w-full border-gray-300 rounded-md text-sm">
                            </div>
                            <div>
                                <label class="block text-xs text-gray-600 mb-1">Unanswered</label>
                                <input type="number" step="0.01" name="marks_unanswered" value="{{ old('marks_unanswered', $examConfiguration->marks_unanswered) }}"
                                       class="block w-full border-gray-300 rounded-md text-sm">
                            </div>
                        </div>
                    </div>
                            ...
                            @foreach ($subjects as $subject)
                                <div class="flex items-center gap-3">
                                    ...
                                    <input type="number" min="0"
                                           ...
                                           value="{{ old('subject_distribution.' . $subject->name, $examConfiguration->subject_distribution[$subject->name] ?? 0) }}"
                    ...
                    <div class="mb-6 flex items-center gap-2">
                        <input type="checkbox" name="is_active" id="is_active" value="1"
                               {{ old('is_active', $examConfiguration->is_active) ? 'checked' : '' }}
